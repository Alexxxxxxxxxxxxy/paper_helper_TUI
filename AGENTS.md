...
## Paper Helper Agent (paper_helper_CLI)

Paper Helper Agent 是存在于 `packages/coding-agent/src/tools/` 下的毕业论文自动化写作系统。相关规则：

### 核心文件

- `packages/coding-agent/src/tools/workflow-controller.ts` — 工作流状态机 + 实验阶段配置
- `packages/coding-agent/src/tools/paper-helper-agent.ts` — Agent 封装，调用自定义模型
- `packages/coding-agent/src/tools/question-classifier.ts` — 问题分类器
- `packages/coding-agent/src/tools/config-loader.ts` — 从 ~/.paper_helper/config.json 加载模型配置

### 模型配置

- 不使用 pi 的 /login 或 env/auth.json 认证
- 模型配置存储在 `~/.paper_helper/config.json`，由 `config-loader.ts` 加载
- 配置格式：`{ provider, model, apiKey, baseUrl, temperature, maxTokens }`
- 通过 `streamSimple()` 的 `apiKey` 选项和 Agent 的 `getApiKey` 回调传递凭证

### 工作流

- 参数提取 → 方向判断 → 实验循环(4阶段+迭代子循环) / 论文撰写(文科)
- 状态机在 `workflow-controller.ts` 的 `WorkflowPhase/ExpPhase/IterPhase/PaperPhase` 中定义
- 每个实验阶段的 prompt 从 `paper_helper.yaml` 的 agent 节点直接提取，必须与 yaml 保持一致
- 迭代子循环：CODE_GEN → BUG_CHECK → PARAM_OPT → EVALUATE（不完整则循环，最多 loopCount 次）

### 修改规范

- 所有 prompt 从 `paper_helper.yaml` 同步，手动写入 `EXP_STAGES` 的 `systemPrompt`
- 更新 yaml 后需要同步更新代码中的 prompt
- 测试时：`cd packages/coding-agent && DEEPSEEK_API_KEY=... npx tsx -e '...'`
- 首次运行前确保 `~/.paper_helper/config.json` 存在且包含有效 apiKey
